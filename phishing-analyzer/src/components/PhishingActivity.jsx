...
const ResultsDialog = ({ open, onClose, results }) => {
  const calculateOverallStats = () => {
    ...
    const totalCorrectLines = results.reduce(
      (sum, result) => sum + result.correctLineCount,
      0
    );

    // Calculate overall accuracy
    const accuracy =
      totalLines > 0 ? (totalCorrectLines / totalLines) * 100 : 0;
    ...
    return {
      ...
      correctlyIdentified: results.reduce(
        (sum, result) => sum + result.truePositives,
        0
      ),
      missedFlags: results.reduce(
          ...
          <Grid item xs={12}>
            <Paper sx={{ p: 2 }}>
              ...
              <Box sx={{ mb: 2 }}>
                ...
                <LinearProgress
                  variant="determinate"
                  value={Math.min(stats.accuracy, 100)}
                ...
                <Typography variant="caption" display="block" align="right">
                  {stats.correctLines} out of {stats.totalLines} lines correct
                </Typography>
              </Box>
                          ...
                          <Box>
                            ...
                            {attempt.isLegitimate ? (
                              <Typography
                                variant="caption"
                                color={
                                  result.correctlyIdentifiedLegitimate
                                    ? "success.main"
                                    : "error.main"
                                }
                              >
                              ...
                              <Typography
                                ...
                                color="text.secondary"
                              >
                                Found: {result.truePositives} | Missed:{" "}
                                {result.falseNegatives} | False Flags:{" "}
